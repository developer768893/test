...
template "/etc/fstab" do
  ...
  source "redis/fstab-appstore-east.erb"
  owner "root"
  group "root"
  mode "0644"
  action :create
  variables(
    ...
    :redis_backup  => 'fs-d77cb39e.efs.us-east-1.amazonaws.com:/ /redis_backup nfs4 defaults,nfsvers=4.1 0 0',
  )
end
...
bash "redis-swaps" do
  ...
  if [ -f /var/lib/redis/swapfile1 ]; then
    ...
    fallocate -l 2G /var/lib/redis/swapfile1
...
bash "redis_backup_dir" do
  ...
  code <<-EOH
   ...
   mkdir -p /opt/tools
   mount -a
   touch /redis_backup/sensu_test_file_dont_remove
   touch /tmp/backup
  EOH
end
...
file "/etc/cron.d/redis_backup" do
  ...
  mode "0644"
  action :create
  content "11 15 * * * root /usr/local/bin/single.py -c /opt/tools/redis_backup.sh
  "
end

template "/etc/redis.conf" do
  only_if { node.chef_environment == "development" }
  source "redis/redis-appstore-east.erb"
  owner "root"
  group "root"
  mode "0644"
  action :create
